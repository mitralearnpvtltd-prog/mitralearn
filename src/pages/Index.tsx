...
// Fallback images for courses without images
...
// Fallback images mapping by title keywords
const fallbackImages: Record<string, string> = {
  'data engineering': courseDataEngineering,
  'ai engineer': courseAiEngineer,
  'fullstack': courseFullstack,
  'full stack': courseFullstack,
  'python': coursePythonAi,
  'java': courseJavaFullstack,
  'marketing': courseMarketing,
};

const getFallbackImage = (title: string): string | undefined => {
...
const Index = () => {
  ...
  const { courses: dbCourses, isLoading: coursesLoading } = usePublishedCourses();
  ...
  // Transform database courses to CourseCardData format
  const courses: CourseCardData[] = dbCourses.map(course => ({
    id: course.id,
    title: course.title,
    description: course.description,
    concepts: course.concepts || [],
    extraConceptsCount: course.extra_concepts_count || 0,
    ...
    badgeColor: course.badge_color || '#7C3AED',
    iconBg: course.icon_bg || '#7C3AED',
    image: course.image_url || getFallbackImage(course.title),
  }));
  ...
  // Hardcoded courses are now replaced by database courses above
